...
class PairwiseComparisonDataset(Dataset):
    ...
    def _create_prompt_template(self, question, response_a, response_b, query_idx):
        ...
        # Build few-shot examples with randomized order for this specific query
        messages = []
...
def parse_args():
    ...
    parser.add_argument("--bt_scores_file", type=str, required=True,
...
def run_experiment_for_seed(seed, args, accelerator, model, tokenizer, token_a_id, token_b_id, dataset):
    ...
    if accelerator.is_main_process:
        ...
        output_file = f"{args.output_dir}/pairwise_preferences_seed_{seed}_k{args.k_shot}.json"
        logger.info(f"Saving results to {output_file}")
        with open(output_file, 'w') as f:
            json.dump(final_results, f, indent=2)

        # Also save metadata about the experiment
        metadata_file = f"{args.output_dir}/metadata_seed_{seed}.json"
        metadata = {
            "seed": seed,
            "k_shot": args.k_shot,
