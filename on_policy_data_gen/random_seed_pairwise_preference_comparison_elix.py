#!/usr/bin/env python3
"""
Script to generate pairwise preference comparisons with in-context learning.

Workflow:
1. Filters dataset by level_id criteria (level_id_x==1, level_id_y==4, scorer_level_id==4)
2. Randomly samples k-shot examples as demonstrations
3. Tests on the remaining samples (up to max_samples)
4. For each query, randomizes the order of demonstrations
5. Generates preference matrix comparing two responses (chosen vs rejected, based on det_choice)
...
def filter_dataset_by_level_ids(dataset):
    """Filter dataset to only include samples with specific level_id criteria.
    ...
    Returns:
        Filtered dataset
    """
    logger.info("Filtering dataset by level_id criteria...")
    logger.info("Criteria: level_id_x==1, level_id_y==4, scorer_level_id==4")

    # Filter dataset based on level_id criteria
    filtered_indices = []
    for i, sample in enumerate(dataset):
        level_id_x = sample.get('level_id_x')
        level_id_y = sample.get('level_id_y')
        scorer_level_id = sample.get('scorer_level_id')

        if level_id_x == 1 and level_id_y == 4 and scorer_level_id == 4:
...
def prepare_persona_dataset_as_pairwise(dataset):
    ...
    The new dataset has 'prompt', 'response_x', 'response_y', and 'det_choice' fields.
    - If det_choice == 1: response_y is preferred (chosen) over response_x (rejected)
    - Otherwise: response_x is preferred (chosen) over response_y (rejected)
    ...
    for sample in dataset:
        ...
        # Get the two responses
        response_x = sample.get('response_x', '').strip()
        response_y = sample.get('response_y', '').strip()
        det_choice = sample.get('det_choice')

        # Determine chosen and rejected based on det_choice
        if det_choice == 1:
            # response_y is preferred over response_x
            chosen = response_y
            rejected = response_x
        else:
            # response_x is preferred over response_y
            chosen = response_x
            rejected = response_y

        # Create pairwise comparison format with 2 responses
        # responses[0] = chosen (winner), responses[1] = rejected (loser)
        converted_data.append({
            'prompt': prompt,
            'all_generated_responses': [chosen, rejected],  # [winner, loser]
...
def run_experiment_for_seed(seed, k_shot, args, accelerator, model, tokenizer, token_a_id, token_b_id, persona_data, persona_id=None):
    ...
    # Use descriptive identifier in filename
    id_str = persona_id if persona_id else "level_filtered"
    part_file = f"{args.output_dir}/seed_{seed}_rank_{rank}_{id_str}.jsonl"
    ...
    if accelerator.is_main_process:
        ...
        for r in range(accelerator.num_processes):
            rank_file = f"{args.output_dir}/seed_{seed}_rank_{r}_{id_str}.jsonl"
        ...
        output_file = f"{args.output_dir}/pairwise_preferences_seed_{seed}_k{k_shot}_{id_str}.json"
        logger.info(f"Saving results to {output_file}")
        with open(output_file, 'w') as f:
            json.dump(final_results, f, indent=2)

        # Also save metadata about the experiment
        metadata_file = f"{args.output_dir}/metadata_seed_{seed}_k{k_shot}_{id_str}.json"
        metadata = {
            "seed": seed,
            "k_shot": k_shot,
            ...
            "filter_criteria": "level_id_x==1, level_id_y==4, scorer_level_id==4",
...
def main():
    ...
    if accelerator.is_main_process:
        logger.info(f"{'='*80}")
        logger.info(f"In-Context Learning Preference Comparison Experiment")
        logger.info(f"{'='*80}")
        logger.info(f"Model: {args.model_name}")
        logger.info(f"Dataset: {args.dataset_name}")
        logger.info(f"Split: {args.split}")
        logger.info(f"Filter: level_id_x==1, level_id_y==4, scorer_level_id==4")
    ...
    # Filter dataset by level_id criteria (no persona filtering)
    if accelerator.is_main_process:
        logger.info(f"\n{'='*80}")
        logger.info(f"Filtering dataset by level_id criteria...")
        logger.info(f"{'='*80}")

    filtered_dataset = filter_dataset_by_level_ids(dataset)
    ...
    if accelerator.is_main_process:
        logger.info("Converting dataset to pairwise comparison format...")
    ...
    total_experiments = len(k_shots_to_run) * len(seeds_to_run)
    if accelerator.is_main_process:
        logger.info(f"\n{'='*80}")
    ...
    # Grid search loop (no persona iteration)
    experiment_count = 0
    for k_shot in k_shots_to_run:
        for seed in seeds_to_run:
            ...
            if accelerator.is_main_process:
                ...
                logger.info(f"K-shot: {k_shot}, Seed: {seed}")
                logger.info(f"{'='*80}")
            ...
            run_experiment_for_seed(
                ...
                persona_data=persona_data,
                persona_id=None  # No persona filtering
            )
